...
def _build_management_section(management_strategy: str | None) -> str:
    ...
    lines = [
        ...
        "- Set 'prices' to contain ONLY the take_profit of THIS specific signal object.",
        "  Do NOT include take_profit values from other signals in the same group.",
        "  Each position watches only its own TP; cascade effects across positions",
        "  (e.g. moving all SLs to BE when TP1 is reached) are triggered automatically",
        "  by the position_closed event when that position closes — do not duplicate",
        "  that trigger in other positions' 'prices' arrays.",
        "- Set 'prices' to null if this signal has no take_profit.",
        "",
    ]
    return "\n".join(lines)
