...
# ── Simulation helpers (imported from shared module) ──────────────────────────
from vps.services.sim_state import (
    _SimState, MockMT5Trader, StatefulMockMT5Trader,
    fire_ai_event as _fire_ai_event_shared,
)
...
async def _fire_ai_event(
    ...
    """Thin wrapper around fire_ai_event that passes sp._client."""
    return await _fire_ai_event_shared(
        gemini_client=sp._client,
        strategy_executor=mock_exec,
        state=state,
...
learning your workflow, not a form asking for inputs."""
    ...
    if step == "sim_analysis":
        ...
        system = (
            ...
            "You are given a detailed event log of what the management agent did at each step. "
            "Write a concise per-signal narrative (3-6 sentences total) that covers:\n"
            "  1. When each signal was entered and at what price.\n"
            "  2. What the agent did at each price-level event — describe the action in plain language "
            "(e.g. 'chiuse il 50% della posizione e spostò lo SL a breakeven') without citing function names.\n"
            "  3. How the remaining position (if partially closed) was ultimately resolved.\n"
            "  4. The final P&L.\n"
            "Be specific — cite prices and what happened, not technical function names. "
